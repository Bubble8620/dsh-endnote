 ...
 * validates args, and forwards to `execute`), and every other third-party plugin
 * here declares the @deepseek-ai packages as OPTIONAL peers — the profile's
 * node_modules does not contain `@deepseek-ai/dsh-tools`, so a hard top-level
 * import would fail at load. When the real one is unavailable we fall back to an
 * equivalent local implementation, so the plugin works either way.
 */
let defineTool
try {
  ...
  : /** Fallback applying the same defaults when schemastery is unavailable. */
    class Config {
      constructor(input = {}) {
        ...
        this.stagingDir = input.stagingDir ?? ''
        this.endnoteExe = input.endnoteExe ?? ''
        this.pythonExe = input.pythonExe ?? ''
        this.toolTimeoutMs = input.toolTimeoutMs ?? 300000
        this.registerTools = input.registerTools ?? true
      }
    }
